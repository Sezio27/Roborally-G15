        ...
        if (subject == player.board) {
            ...
            if (player.board.getPhase() != Phase.PLAYER_INTERACTION) {
                ...
                if (player.board.getCurrentPlayer() == player) {
                    ...
                    // Access current card and its command options
                    CommandCard card = player.getProgramField(player.board.getStep()).getCard();
                    Command command = card.command;

                    //  Loop through options and add buttons
                    for (Command option: command.getOptions()) {
                        Button optionButton = new Button(option.displayName);
                        optionButton.setOnAction(e -> gameController.executeCommandOptionAndContinue(player, option));
